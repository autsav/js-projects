...
form.addEventListener("submit", function(e){
  //The default behaviour of form is to sent the data to server so we prevent that by:
  e.preventDefault();
...
//if empty or
// if -1 or
// >9  the generate only the first paragraph
if(isNaN(value) || value < 0 || value >9){
  result.innerHTML = `<p class="result">${text[random]}</p>`;
}else{
  //slice grabs value between index that start from certain number to certain number
  let tempText = text.slice(0, value);
  //Add single paragraph to step wise multiple paragraph
